...
Combines traditional volatility measures with news-based sentiment indicators following behavioral finance principles \cite{Daniel1998,Campbell2001}. Volatility is calculated using rolling standard deviation of percentage changes over recent trading days, following established GARCH methodologies \cite{Bollerslev1986,Engle1982}. The framework implements regime-dependent behavior with negative bias for high volatility periods reflecting flight-to-quality effects and positive bias for low volatility enhanced by news sentiment \cite{French1987}.

\subsubsection{{Size Factor}}

Extends size effect research \cite{Banz1981} by incorporating financial impact extracted from news using NLP techniques \cite{Loughran2011}. The size factor employs market capitalization changes relative to historical averages, following established methodologies in asset pricing literature \cite{FAMA1993}. News impact is quantified through financial keyword extraction and entity recognition to capture market value fluctuations driven by sentiment \cite{Xing2018}.
...
Captures investment activity effects \cite{Daniel1998} through investment amounts and activity classification with sector-specific weightings for acquisition, expansion, and R\&D activities \cite{Kothari2009}. The factor implementation incorporates conditional scaling based on news sentiment polarity, following established practices in event-driven asset pricing models \cite{Ball1968}.
...
The sentiment integration follows established practices in combining multiple sentiment measures \cite{Loughran2011,TETLOCK2007}. The methodology implements weighted averaging of TextBlob polarity scores and keyword-based sentiment analysis, with sentiment-dependent scaling reflecting documented non-linear relationships between sentiment strength and market response \cite{Daniel1998}. Final amplification ensures adequate signal strength while maintaining interpretable bounds.
...
The news effect factor employs multiplicative amplification to ensure adequate signal strength while maintaining bounded interpretability, following established practices in sentiment-based financial modeling \cite{TETLOCK2007,Schumaker2009}.
...
% FinReport successfully integrates technical indicators, financial news sentiment, and advanced risk metrics to deliver interpretable and accurate stock earnings forecasts, outperforming baseline models especially in large-cap and consumer stocks. The system demonstrates strong predictive capability with R² of 0.5515 and correlation of 0.948, indicating substantial explanatory power for stock return variance . 

The news sentiment analysis using FinBERT revealed a consistent contrarian effect across Chinese markets, where negative sentiment precedes positive returns, supporting behavioral finance theories of market overreaction . Risk assessment integration using EGARCH volatility modeling and CVaR calculations  provides comprehensive risk-adjusted forecasting capabilities.

Future enhancements will focus on incorporating advanced Large Language Models  for improved news analysis, dynamic factor weighting, and intelligent report generation, alongside scalability and regulatory compliance improvements. 

% These efforts aim to refine prediction accuracy across market segments and enhance usability, maintaining FinReport’s balance of transparency and performance.
